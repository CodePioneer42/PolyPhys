...
*   **Justification:**[Briefly explain why this specific engine is the best fit for the user's confirmed mechanism, scale, and active/passive requirements.]
*   **Key Modules & Command Mapping:**
    *   **Polymer Topology/Bonds:**[Explicitly name the command/style, e.g., `bond_style fene` or `hoomd.md.bond.FENE`]
    *   **Specific Interactions (Non-bonded):**[Explicitly name the command, e.g., `pair_style lj/cut` or `pair_style yukawa`]
    *   **Integrator & Dynamics:**[Explicitly name the integrator, e.g., `fix langevin` for Brownian dynamics, or specific Active Matter plugins if needed]
*   **Limitations & Warnings:**[Identify potential bottlenecks, missing features, or computational costs when using this software for the current model.]
...
3. Evaluate at least one or two concrete mainstream engines such as `LAMMPS`, `HOOMD-blue`, `OpenMM`, or `GROMACS`.
...
5. Map the current model ingredients onto explicit bond, non-bonded, and integrator commands or APIs.
6. State the `Software Capability Check`, `Fidelity / Cost Tradeoff`, `Explicit Solvent / Ion Note`, and `Limitations & Warnings` explicitly.
7. Keep the recommendation conservative and tied to the active ingredients that matter.
...
- Recommend one or two concrete engines rather than an engine class.
- Include explicit command or API mappings for bonds, non-bonded interactions, and dynamics.
- State whether the route is `package-native` or `package + custom extension`.
...
- Choose the `Recommended Paradigm` and one or two `Recommended Engine` candidates.
...
- State the `Software Capability Check`, `Fidelity / Cost Tradeoff`, `Explicit Solvent / Ion Note`, and `Limitations & Warnings`.
...
- If recommendation is ready, present the conservative route using the mandatory engine block format.
...
## Self-Review
- `stage completeness`: did the response include readiness, route, capability check, tradeoff, solvent/ion assumptions, and command mapping?
...
- `command specificity`: did the output name real commands or APIs instead of generic engine classes?
...
- Whether the route is `package-native` or `package + custom extension`
...
Use one or two engine blocks, exactly in this format:
...
- Recommending abstract fallbacks such as "standalone custom code", "write your own scripts", or "generic MD engine".
...
- Naming a package without giving command or API mappings for the actual model ingredients.
